...
@Component({
    ...
    standalone: true,
    imports: [CommonModule, FormsModule, ReactiveFormsModule, NumberFormatPipe, MatIconModule],
        ...
        this.o_service.CreateOrder(this.currentOrderDTO).subscribe((data: any) => {
            if (data.statusCode == 200) {
                ...
                toastRef.onTap.subscribe(() => {
                    this.router.navigate(['/entry/update-order'], {
                        queryParams: { orderID: data?.data?.[0]?.orderId },
                    });
