...
export class TransportSalaryComponent implements OnInit, OnDestroy {
    ...
    private updateFilteredLocationsForType(
        dropdownType: 'fromLocation' | 'fromWhere' | 'toLocation',
        data: LocationDTO[],
        context: 'modal' | 'filter',
    ) {
        if (context === 'modal') {
    ...
    private updateSourceDataForDropdownType(
        dropdownType: 'fromLocation' | 'fromWhere' | 'toLocation',
        data: LocationDTO[],
    ) {
        switch (dropdownType) {
    ...
    loadLocationForType(dropdownType: 'fromLocation' | 'fromWhere' | 'toLocation', context: 'modal' | 'filter') {
        ...
        this.l_service.listLocation(this.token, 'id', 'asc', 100, 1, searchKey).subscribe((data: any) => {
            if (data.statusCode == 200) {
