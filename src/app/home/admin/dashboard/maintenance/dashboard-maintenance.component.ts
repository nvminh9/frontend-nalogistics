...
@Component({
    ...
    standalone: true,
    imports: [FormsModule, ReactiveFormsModule, CommonModule, StringDateFormatPipePipe, MatIcon, MatIconModule],
...
export class DashboardMaintenanceComponent {
    ...
    getSelectedMaintenanceTypeName(): string {
        const type = this.listMaintenanceTypeDTO.find(
            (t: any) => t.maintenanceTypeID === this.MaintenanceForm.MaintenanceTypeID,
        );
    ...
    getSelectedMaintenanceTypeNameDetail(): string {
        if (!this.currentMaintenance) return '';
        const type = this.listMaintenanceTypeDTO.find(
            (t: any) => t.maintenanceTypeID === this.currentMaintenance?.maintenanceTypeID,
        );
        return type ? type.nameType : '';
    }
