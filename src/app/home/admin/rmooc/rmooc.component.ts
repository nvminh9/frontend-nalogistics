...
import { RmoocService } from '../../../../services/rmooc-service';
...
@Component({
  ...
  standalone: true,
  templateUrl: './rmooc.component.html',
  styleUrl: './rmooc.component.css'
})
export class RmoocComponent {
  constructor(
    private t_service: RmoocService,
  ...
  ngOnInit() {
    this.t_service.listRmooc(this.token, this.order, this.sortBy, this.pageSize, this.pageNumber, this.keySearch).subscribe((data: any) => {
  ...
  DetailTruck(TruckID: number) {
    this.t_service.DetailRmooc(TruckID).subscribe((data: any) => {
  ...
  updateTruckFromDetail(TruckID: number) {
    ...
    this.t_service.UpdateRmooc(TruckID, truckDTO).subscribe((data: any) => {
      if (data.statusCode == 200) {
  ...
  saveTruck() {
    ...
    if (this.isEditMode && this.currentTruck) {
      ...
      this.t_service.CreateRmooc(truckDTO).subscribe((data: any) => {
        if (data.statusCode == 200) {
          this.toastr.success(data.message)
          this.closeAddModal();
